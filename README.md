...
**숲나들e(산림청 자연휴양림)** 와 **숲이랑(한국산림복지진흥원 산림복지시설)** 의
**빈자리**를 조회하고, 원하는 조건에 빈자리가 생기면 알림을 보내주는 웹 앱입니다.
취소표(빈자리) 잡기에 유용합니다.

- 전국 자연휴양림(국립/공립/사립) 실시간 빈자리 조회
- 시설종류 필터: **숲속의 집 · 야영장 · 카라반**(숲나들e) + **숲이랑**(국립숲체원·산림치유원)
- 지역 / 휴양림 / 날짜(주말 토·일) 필터
...
| 소스 | 용도 | 상태 |
|---|---|---|
| **숲나들e 내부 조회 API** (`foresttrip.go.kr`) | 전국 147개 휴양림 실시간 빈자리(숲속의집·야영장·카라반) | ✅ 기본 사용 |
| **숲이랑 월별예약조회** (`sooperang.go.kr`) | 국립숲체원·산림치유원 등 산림복지시설 숙박 빈자리 | ✅ 기본 사용 |
| **공공데이터포털 OpenAPI** (국립 예약정보) | 국립 휴양림 교차검증 | ⚙️ 선택(키 필요) |

**카라반**은 숲나들e의 휴양림별 상품분류(goodsClssc, 이름에 ‘카라반’ 포함)로, 숙박·야영 양쪽에
걸쳐 있어 휴양림별로 필터 조회합니다(`server/caravan.js`, `data/caravan-map.json` 캐시).

**숲이랑**(`server/sooperangClient.js`)은 시설별 `월별예약조회` 페이지에 임베드된
`gsrmList`(객실×일자, `rsrvtYn=="Y"`=예약가능)를 스트리밍으로 파싱합니다. 예약가능 창은
약 1개월 롤링이라 그 이후 날짜는 ‘미오픈’으로 표시됩니다. 예약 신청은 로그인이 필요합니다.
...
**🔄 지금 업데이트 버튼** — 30분 자동 갱신 외에 원할 때 즉시 업데이트:
- **로컬에서 대시보드를 열면**(`npm start` → http://localhost:3000/board) 이 버튼이
  숲나들e를 **지금 실시간 조회**하고 결과를 gh-pages까지 배포합니다(수 분 소요).
- **공개 gh-pages** 에서는 실시간 조회가 불가하여, 버튼이 **최신 스냅샷을 다시 불러오고**
  실시간 갱신은 로컬에서 하라고 안내합니다.
